    ...
    <Step title="Upload the data" stepNumber="3">
        ...
        <Tabs>
            ...
            <Tab title="HTTP Gateway with webhooks">
                ...
                ```bash
                curl --request POST \
                  --url "https://http-gateway.vatis.tech/http-gateway/api/v1/upload?id=$STREAM_ID&streamConfigurationTemplateId=$CONFIG_ID&persist=true&language=en&webhook.stream.completed=http://my_domain/success&webhook.stream.failed=http://my_domain/fail" \
                  --header "Authorization: Basic $API_KEY" \
                  --header 'Content-Type: application/octet-stream' \
                  --data-binary @"$FILE_PATH" | jq
                ```
